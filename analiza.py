...
def run_analysis(lake_name="Isąg"):
    ...
    def fetch_lake_polygon_wkt(lake_name, country="Poland"):
        ...
        Args:
            lake_name (str): The name of the lake to find
            country (str): The country where the lake is located

        Returns:
            tuple: (WKT string of the lake polygon, lake geometry object)
        """
        import osmnx as ox
        import geopandas as gpd
        ...
        # Get all water features in the area
        tags = {"natural": "water"}
        gdf = ox.features_from_place(place_name, tags)

        # Filter only for polygons/multipolygons
        gdf = gdf[gdf.geometry.type.isin(["Polygon", "MultiPolygon"])]

        if gdf.empty:
            raise ValueError(f"No water polygons found near '{lake_name}' in {country}")

        # Filter for the specific lake name in the 'name' column
        # Check multiple name columns that OSM might use
        name_columns = ['name', 'name:en', 'alt_name', 'official_name']

        # Create a mask to find exact matches for the lake name
        exact_match_mask = False
        for col in name_columns:
            if col in gdf.columns:
                # Case-insensitive exact match
                exact_match_mask |= gdf[col].str.lower() == lake_name.lower()

        # Apply the exact match filter
        exact_matches = gdf[exact_match_mask]

        if not exact_matches.empty:
            # Found exact matches by name
            geom = exact_matches.geometry.iloc[0]
            return geom.wkt, geom

        # If no exact matches, try fuzzy matching as a fallback
        import difflib

        # Collect all names from the dataset
        all_names = []
        for col in name_columns:
            if col in gdf.columns:
                all_names.extend(gdf[col].dropna().tolist())

        # Find the closest match
        if all_names:
            closest_matches = difflib.get_close_matches(lake_name, all_names, n=1, cutoff=0.6)

            if closest_matches:
                closest_name = closest_matches[0]
                print(f"Exact match not found. Using closest match: '{closest_name}'")

                # Find the geometry for the closest match
                for col in name_columns:
                    if col in gdf.columns:
                        matches = gdf[gdf[col] == closest_name]
    ...
    # -----------------------------------
    # STEP 5: Plotting and Extract Data for Analysis
    # -----------------------------------
    ...
    def create_prediction_model(eopatch, forecast_days=365, filename=None):
        ...
        print(f"Model R^2 score: {test_score:.4f} (train: {train_score:.4f})")
    ...
    def plot_water_levels(eopatch, max_coverage=1.0, filename=None):
        ...
        # Zwracamy dane potrzebne do analizy
        return valid_water_levels
    ...
    def plot_annual_comparison(eopatch, filename=None):
        ...
        # Plot each year with rainbow colors
    ...
    def modified_monthly_averages(eopatch, filename=None):
        ...
        # Set y-axis minimum based on lowest data point
        # Find the minimum value considering both means and error bars
        min_means = monthly_avg['mean'][~np.isnan(monthly_avg['mean'])]
        min_with_err = min_means - monthly_avg['std'][~np.isnan(monthly_avg['std'])]
        min_with_err = min_with_err[~np.isnan(min_with_err)]

        if len(min_with_err) > 0:
            lowest_point = min(min_with_err)
        elif len(min_means) > 0:
            lowest_point = min(min_means)
        else:
            # Fallback if all values are NaN
            lowest_point = 0.8

        # Calculate the floor to nearest 0.1
        min_val = math.floor(lowest_point * 10) / 10
    ...
    # -----------------------------------
    # STEP 6: Save Results and Analyze Data
    # -----------------------------------
